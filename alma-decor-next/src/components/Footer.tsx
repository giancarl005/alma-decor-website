    ...
    <footer className="bg-gray-50 dark:bg-[#0a0a0a] border-t border-gray-100 dark:border-white/[0.03] transition-colors duration-500">
      <div className="max-w-7xl mx-auto px-6 py-24">
          ...
          <div className="space-y-8">
            ...
            <ul className="space-y-4">
              {[
                ...
                { name: 'Politică Confidențialitate', path: '/politica-de-confidentialitate' },
                { name: 'Politică Cookie', path: '/politica-de-cookies' },
                { name: 'ANPC', path: 'https://anpc.galati.ro/' }
              ].map((item) => (
                <li key={item.name}>
                  <Link 
                    href={item.path}
                    target={item.name === 'ANPC' ? '_blank' : '_self'}
                    className="text-gray-500 dark:text-gray-400 text-sm font-medium hover:text-brand-yellow transition-colors flex items-center gap-2 group"
                  >
                    <span className="w-0 h-px bg-brand-yellow transition-all group-hover:w-3"></span>
                    {item.name}
        ...
        <div className="mt-20 pt-8 border-t border-gray-100 dark:border-white/[0.03] flex flex-col md:flex-row justify-between items-center gap-6">
          <p className="text-gray-400 dark:text-gray-600 text-[10px] font-bold uppercase tracking-[0.2em] text-center md:text-left">
            © {new Date().getFullYear()} ALMA DECOR. TOATE DREPTURILE REZERVATE.
          </p>
          <div className="flex flex-row gap-4 items-center justify-center">
             <a href="https://anpc.ro/ce-este-sal/" target="_blank" rel="nofollow" className="hover:scale-105 transition-transform">
               <img src="/images/anpc-sal.png" alt="ANPC SAL" className="h-12 sm:h-14 w-auto object-contain" />
             </a>
             <a href="https://consumer-redress.ec.europa.eu/index_ro" target="_blank" rel="nofollow" className="hover:scale-105 transition-transform">
               <img src="/images/anpc-sol.png" alt="ANPC SOL" className="h-12 sm:h-14 w-auto object-contain" />
             </a>
          </div>
        </div>
      </div>
