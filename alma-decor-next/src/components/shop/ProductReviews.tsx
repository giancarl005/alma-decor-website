...
const ProductReviews: React.FC<ProductReviewsProps> = ({ productId }) => {
  ...
  const fetchReviews = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/recenzii.php?product_id=${productId}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/recenzii.php`, {
        method: 'POST',
        body: form
      });
      const data = await res.json();
    ...
    <div className="space-y-16">
      {stats && (
        <div className="bg-white dark:bg-white/[0.02] p-10 md:p-12 rounded-3xl border border-gray-100 dark:border-white/5 flex flex-col md:flex-row items-center gap-12">
            ...
            <div className="flex-grow space-y-2.5 md:border-r border-gray-100 dark:border-white/5 md:pr-12 py-2 w-full">
              ...
              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-12">
                ...
                <div className="space-y-8 flex flex-col">
                  ...
                  <button
                    disabled={submitting}
                    type="submit"
                    className="w-full bg-brand-yellow text-gray-900 h-14 rounded-full font-black text-[11px] uppercase tracking-[0.3em] hover:bg-gray-900 hover:text-white transition-all shadow-xl shadow-brand-yellow/10"
                  >
                ...
                <div key={review.id} className="bg-white dark:bg-white/[0.01] p-8 rounded-[2rem] border border-gray-50 dark:border-white/5 space-y-6 hover:shadow-lg transition-all duration-300 group">
                  ...
                  {review.imagini && review.imagini.length > 0 && (
                    <div className="flex flex-wrap gap-2 pt-2">
                      {review.imagini.map((img: any, i: number) => (
                        <div key={i} className="w-16 h-16 rounded-lg overflow-hidden border border-gray-100 dark:border-white/5">
                          <img src={getFullImageUrl(img.url || img)} alt="User upload" className="w-full h-full object-cover cursor-pointer hover:scale-110 transition-transform" />
                        </div>
                      ))}
                    </div>
                  )}
