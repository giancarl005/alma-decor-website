...
const Search: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    const searchTimeout = setTimeout(async () => {
      ...
      try {
        ...
        let apiUrl = `${API_BASE}/api/produse.php?q=${encodeURIComponent(query)}&limit=20`;
  ...
  return (
    <div className="fixed inset-0 z-[200] flex flex-col bg-white dark:bg-[#0F1115] animate-staggered-fade overflow-y-auto">
      <div className="container mx-auto px-6 py-8 flex flex-col min-h-full">
                  ...
                  {results.map((product) => (
                    ...
                    >
                      <div className="w-20 h-20 bg-gray-50 dark:bg-white/5 rounded-xl overflow-hidden flex-shrink-0">
                        <img 
                          src={product.primary_image?.startsWith('http') ? product.primary_image : `${API_BASE}${product.primary_image}`} 
                          alt={product.name} 
