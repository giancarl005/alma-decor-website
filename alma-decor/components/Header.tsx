...
const Header: React.FC = () => {
  ...
  const isCheckout = location.pathname === '/comanda';
  ...
  const navLinks = [
    ...
    { name: 'Procesul', id: 'procesul-nostru', type: 'scroll' },
    { name: 'Magazin', path: '/magazin', type: 'link' },
    { name: 'Portofoliu', id: 'portofoliu', type: 'scroll' },
  ...
  const separatorClass = isScrolled || !isHomePage
          ...
          {isCheckout ? (
            <>
              ...
              <div className="flex-shrink-0 absolute left-1/2 -translate-x-1/2">
                <Link to="/" className="flex items-center text-xl sm:text-2xl font-black tracking-tighter">
                  <span className="text-gray-900 dark:text-white">ALMA</span>
                  <span className="text-brand-yellow ml-1">DECOR</span>
                </Link>
              </div>
            ...
            <>
              ...
              <div className="flex-shrink-0">
                <Link to="/" className="flex items-center text-xl sm:text-2xl font-black tracking-tighter">
                  <span className={isScrolled || !isHomePage ? 'text-gray-900 dark:text-white' : 'text-white'}>ALMA</span>
                  <span className="text-brand-yellow ml-1">DECOR</span>
                </Link>
              </div>
                      ...
                      {link.type === 'link' ? (
                        <Link
                          ...
                          className={`hover:text-brand-yellow transition-colors duration-300 font-bold text-[13px] uppercase tracking-wider cursor-pointer ${
                            location.pathname === link.path ? 'text-brand-yellow' : (isScrolled || !isHomePage ? 'text-gray-900 dark:text-white' : 'text-white')
                          }`}
                        >
                          {link.name}
                        </Link>
                        ...
                        <a
                          ...
                          className={`hover:text-brand-yellow transition-colors duration-300 font-bold text-[13px] uppercase tracking-wider cursor-pointer ${
                            activeSection === link.id ? 'text-brand-yellow' : (isScrolled || !isHomePage ? 'text-gray-900 dark:text-white' : 'text-white')
                          }`}
                        >
                          {link.name}
                        </a>
              ...
              <div className="flex items-center space-x-0.5 sm:space-x-2">
                  ...
                  <button 
                    ...
                    className={`p-2 transition-colors duration-300 hover:text-brand-yellow ${isScrolled || !isHomePage ? 'text-gray-900 dark:text-white' : 'text-white'}`}
                    aria-label="Search"
                  >
                  ...
                  <Link 
                    to={localStorage.getItem('alma_customer_token') ? "/cont/dashboard" : "/cont/login"} 
                    className={`p-2 transition-colors duration-300 hover:text-brand-yellow ${isScrolled || !isHomePage ? 'text-gray-900 dark:text-white' : 'text-white'}`}
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <Link 
                    to="/cos" 
                    className={`relative p-2 transition-colors duration-300 group hover:text-brand-yellow ${isScrolled || !isHomePage ? 'text-gray-900 dark:text-white' : 'text-white'}`}
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <a
                href="#contact"
                onClick={(e) => handleNavClick(e, { type: 'scroll', id: 'contact' })}
              ...
              {/* Mobile Menu Toggle */}
              <button className={`lg:hidden p-2 ml-1 ${isScrolled || !isHomePage ? 'text-gray-900 dark:text-white' : 'text-white'}`} onClick={() => setIsMenuOpen(true)} aria-label="Open menu">
        ...
        >
            <div className="flex justify-between items-center p-6 border-b border-gray-100 dark:border-white/5">
                <div className="flex items-center gap-4">
                  <Link to="/" onClick={() => setIsMenuOpen(false)} className="text-xl font-bold flex items-center tracking-tighter">
                      <span className="text-gray-900 dark:text-white">Alma</span>
                      <span className="text-brand-yellow ml-0.5">Decor</span>
                  </Link>
                  <ThemeToggleButton />
                </div>
